...
interface Props { place: Place | null; onClose: () => void; onSuccess: () => void }

export default function PlaceModal({ place, onClose, onSuccess }: Props) {
  const isEdit = !!place
  const { categories, isLoading: catLoading } = useCategories('place')
  ...
  const mutation = useMutation({
    mutationFn: (data: Partial<Place>) => isEdit ? placeApi.update(place!.id, data) : placeApi.create(data),
    onSuccess: () => { toast.success(isEdit ? 'Place updated!' : 'Place created!'); onSuccess() },
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold text-gray-900">{isEdit ? 'Edit Place' : 'Add Place'}</h2>
              <p className="text-xs text-gray-400">{isEdit ? `Editing ${place.name}` : 'Fill in the details below'}</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={form.name} onChange={e => set('name', e.target.value)} required placeholder="e.g. DHQ Hospital"
          ...
          <div>
            ...
            <textarea value={form.description} onChange={e => set('description', e.target.value)} rows={3} placeholder="Describe this place..."
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">Facilities <span className="normal-case font-normal text-gray-400">(comma separated)</span></label>
            <input value={form.facilities} onChange={e => set('facilities', e.target.value)} placeholder="e.g. Emergency, OPD, Pharmacy"
              className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-400" />
          </div>

          <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl">
            <div onClick={() => set('isFeatured', !form.isFeatured)}
              className={`relative w-10 h-5 rounded-full cursor-pointer transition-colors ${form.isFeatured ? 'bg-amber-500' : 'bg-gray-300'}`}>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={mutation.isPending}
              ...
              {mutation.isPending ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Saving...</> : isEdit ? '✓ Update Place' : '+ Add Place'}
            </button>
          </div>
        </form>
      </div>
