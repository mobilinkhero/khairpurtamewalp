...
export default function BusinessesTable() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => businessApi.getAll({ ...(search && { search }), page: String(page), limit: '10' }),
  })
  ...
  const openEdit = (b: Business) => { setEditing(b); setModalOpen(true) }
  ...
  const toggleAll = () => {
    ...
    setSelected(new Set(data.data.map((b) => b.id)))
  }
  ...
  const bulkAction = useCallback(async (action: 'publish' | 'unpublish' | 'delete') => {
    ...
    if (action === 'delete' && !confirm(`Delete ${selected.size} businesses?`)) return
    setBulkLoading(true)
    try {
      const res = await fetch('/api/admin/bulk', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ entity: 'businesses', ids: Array.from(selected), action }),
      })
      if (!res.ok) throw new Error()
      toast.success(`${action === 'delete' ? 'Deleted' : action === 'publish' ? 'Published' : 'Unpublished'} ${selected.size} businesses`)
      setSelected(new Set())
      qc.invalidateQueries({ queryKey: ['admin', 'businesses'] })
  ...
  const exportCSV = useCallback(() => {
    ...
    const headers = ['Name', 'Category', 'Description', 'Address', 'Phone', 'Rating', 'ReviewCount', 'isOpen', 'WorkingHours', 'isFeatured', 'Tags']
    const rows = data.data.map((b) => [b.name, b.category, `"${(b.description || '').replace(/"/g, '""')}"`, `"${b.address}"`, b.phone, b.rating, b.reviewCount, b.isOpen, b.workingHours, b.isFeatured, `"${b.tags || ''}"`])
    const csv = [headers.join(','), ...rows.map((r) => r.join(','))].join('\n')
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a'); a.href = url; a.download = 'businesses.csv'; a.click()
    ...
    <div className="space-y-5">
          ...
          {selected.size > 0 && (
            <div className="flex items-center gap-1.5 bg-green-50 border border-green-200 rounded-xl px-3 py-2 whitespace-nowrap">
              ...
              <button onClick={() => bulkAction('publish')} disabled={bulkLoading}
                className="px-2.5 py-1 text-xs font-semibold bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 transition-colors">Publish</button>
              <button onClick={() => bulkAction('unpublish')} disabled={bulkLoading}
                className="px-2.5 py-1 text-xs font-semibold bg-amber-500 text-white rounded-lg hover:bg-amber-600 disabled:opacity-50 transition-colors">Unpublish</button>
              <button onClick={() => bulkAction('delete')} disabled={bulkLoading}
                className="px-2.5 py-1 text-xs font-semibold bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:opacity-50 transition-colors">Delete</button>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="divide-y divide-gray-50">
          {isLoading ? (
            [...Array(6)].map((_, i) => (
              <div key={i} className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_80px] gap-4 px-5 py-4 animate-pulse">
            ...
            <div className="py-16 text-center">
              ...
              <p className="text-xs text-gray-400 mt-1">Try a different search or add a new one</p>
            </div>
          ) : (
            data?.data.map((b) => (
              <div key={b.id} className="grid grid-cols-[40px_2fr_1fr_1fr_1fr_1fr_80px] gap-4 px-5 py-4 hover:bg-gray-50/60 transition-colors items-center group">
                ...
                <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold w-fit ${
                  b.isOpen ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-600'
                }`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${b.isOpen ? 'bg-green-500' : 'bg-red-400'}`} />
                  {b.isOpen ? 'Open' : 'Closed'}
                </span>

                {/* Featured */}
                <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold w-fit ${
                  b.isFeatured ? 'bg-amber-50 text-amber-700' : 'bg-gray-100 text-gray-400'
                }`}>
                  {b.isFeatured ? '★ Featured' : '— Normal'}
                </span>
        ...
        {data && data.pagination.totalPages > 1 && (
          <div className="flex items-center justify-between px-5 py-3.5 border-t border-gray-100 bg-gray-50/50">
            <p className="text-xs text-gray-500">
              Showing <span className="font-semibold text-gray-700">{(page - 1) * 10 + 1}–{Math.min(page * 10, data.pagination.total)}</span> of <span className="font-semibold text-gray-700">{data.pagination.total}</span> businesses
            </p>
            <div className="flex items-center gap-1">
              <button
                onClick={() => setPage((p) => p - 1)}
                disabled={!data.pagination.hasPrev}
                className="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-200 disabled:opacity-40 hover:bg-white transition-colors text-gray-600"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {[...Array(data.pagination.totalPages)].map((_, i) => (
                <button
                  ...
                  className={`w-8 h-8 rounded-lg text-xs font-semibold transition-colors ${
                    page === i + 1 ? 'bg-green-600 text-white' : 'hover:bg-white border border-gray-200 text-gray-600'
                  }`}
                >
                  {i + 1}
                </button>
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl">
        ...
        <h3 className="text-lg font-bold text-gray-900 text-center">Delete Business</h3>
        <p className="text-sm text-gray-500 text-center mt-2">Are you sure you want to delete <span className="font-semibold text-gray-700">"{name}"</span>? This action cannot be undone.</p>
        <div className="flex gap-3 mt-6">
          ...
          <button onClick={onConfirm} disabled={loading} className="flex-1 py-2.5 bg-red-600 hover:bg-red-700 rounded-xl text-sm font-semibold text-white transition-colors disabled:opacity-60 flex items-center justify-center gap-2">
